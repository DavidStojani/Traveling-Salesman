...
public class DelaunayTriangulator {
    ...
    public void triangulate() {
        triangleSoup = new TriangleSoup();
        //Exeption if less than 3 points
        ...
        for (Point2D vector : getPointSet()) {
        ...
        for (int i = 0; i < pointSet.size(); i++) {
            ...
            if (triangle == null) {
                ...
                Point2D firstNoneEdgeVertex = first.getNoneEdgeVertex(edge);
                Point2D secondNoneEdgeVertex = second.getNoneEdgeVertex(edge);
     ...
     */
    private void legalizeEdge(Triangle2D triangle, Edge2D edge, Point2D newVertex) {
        ...
        if (neighbourTriangle != null) {
            if (neighbourTriangle.isPointInCircumcircle(newVertex)) {
                ...
                Point2D noneEdgeVertex = neighbourTriangle.getNoneEdgeVertex(edge);
